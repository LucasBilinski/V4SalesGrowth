...
async function executeSQLDirectly(scriptName, sqlContent) {
  try {
    console.log(`🔄 Executando diretamente: ${scriptName}`);
    ...
    for (let i = 0; i < commands.length; i++) {
      ...
      if (response.ok) {
        ...
        console.log(`    ⚠️ Comando ${i + 1} - Aviso: ${error}`);
      }
...
async function setupDatabase() {
  ...
  console.log('📋 MÉTODO: Execução direta via API REST do Supabase\n');
  ...
  for (let i = 0; i < scriptFiles.length; i++) {
    ...
    if (fs.existsSync(scriptPath)) {
      ...
      const success = await executeSQLDirectly(scriptFile, sqlContent);
